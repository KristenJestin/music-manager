...
describe("the recorded scenarios", () => {
  it("holds the four the phase specification names", () => {
    expect(cassetteNames()).toEqual(["currents", "discovery", "formidable", "skinny-love"]);
  });
...
describe("the request budget", () => {
  it("spends at most two searches and N lookups on an album", async () => {
  ...
  it("spends the same budget on Currents", async () => {
    ...
    const gateway = cassetteGateway(recorded);
    const result = await matchAlbum(gateway, albumInput(recorded), settings);
    expect(result.budget).toEqual({ searches: 2, lookups: 6 });
  });
